...
class DoctorDashboard(APIView):
    ...
    def get(self, request, format=None):
        ...
        # Ensure the correct status is used
        today_appointment = PatientBooking.objects.filter(
            doctor=doctor,
            appointment_date=localdate(),  # Ensures correct date handling
            status='upcoming'  # Make sure this status exists
        ).select_related('patient')

        serialized_data = PatientBookingSerializer(today_appointment, many=True).data
        print("Today's Appointments:", serialized_data)  # Debugging Output
